...
def track_linear_velocity_z(
  ...
  """Reward for tracking the commanded base z velocity.
  """
  asset: Entity = env.scene[asset_cfg.name]
  command = env.command_manager.get_command(command_name)
  assert command is not None, f"Command '{command_name}' not found."
  actual = asset.data.root_link_lin_vel_b
  lin_vel_error = torch.square(command[:, 3] - asset.data.root_link_lin_vel_w[:, 2])
  return torch.exp(-lin_vel_error / std**2)
...
def undesired_velocity(
  ...
  asset: Entity = env.scene[asset_cfg.name]
  vel = asset.data.body_link_lin_vel_b[:, asset_cfg.body_ids, :]
  commands = env.command_manager.get_command(command_name)
  # Penalize residual x/y velocity and yaw rate when their commands are near zero.
  xy_mask = (torch.abs(commands[:, :2]) < command_threshold).unsqueeze(1)
  xy_penalty = torch.sum(torch.abs(vel[:, :, :2]) * xy_mask, dim=(1, 2))

  z_mask = (torch.abs(commands[:, 3]) < command_threshold).unsqueeze(1)
  z_penalty = torch.sum(torch.abs(vel[:, :, 2]) * z_mask, dim=(1, 2))

  yaw_vel = asset.data.body_link_ang_vel_w[:, asset_cfg.body_ids, 2]
  yaw_mask = (torch.abs(commands[:, 2]) < command_threshold).unsqueeze(1)
  yaw_penalty = torch.sum(torch.abs(yaw_vel), dim=1) * yaw_mask

  return xy_penalty + z_penalty + 2.0 * yaw_penalty
...
def undesired_stepping(
  ...
  no_contact = sensor_data.contact_forces < 50.0
  commands = env.command_manager.get_command(command_name)
  # Penalize stepping when the command is near zero.
  mask = (torch.norm(commands[:, :3]) < command_threshold).unsqueeze(1)
  if commands is None or torch.norm(commands[:, :3]) >= command_threshold:
    return torch.zeros(env.batch_size, device=env.device)
  penalty = torch.sum(no_contact.float() * mask, dim=(1, 2))
  return penalty
