...
class UniformVelocityCommand(CommandTerm):
  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    self.vel_command_b[env_ids, :] *= (torch.norm(self.vel_command_b[env_ids, :3], dim=1) > 0.1).unsqueeze(1)
    ...
    if len(init_vel_env_ids) > 0:
      ...
      # Set the world-frame vertical velocity without changing the commanded
      # horizontal velocities in body frame.
  ...
  def _debug_vis_impl(self, visualizer: "DebugVisualizer") -> None:
    ...
    for batch in env_indices:
      ...
      # Offset it so it does not cover the commanded vertical velocity arrow.
